...
PCMSolver_TMPDIR=/Users/roberto/Scratch/RDR-clang3.5-release
mkdir -p $PCMSolver_TMPDIR
export PCMSolver_TMPDIR
export NPROCS=`sysctl -n hw.ncpu`
export CTEST_MAKE_NUM_PROCS=$NPROCS

TMP_DIR=/Users/roberto/Scratch/tmprunpcmsolver/RDR-clang3.5-release
mkdir -p $TMP_DIR
...
python setup.py --fc=gfortran --cc=clang --cxx=clang++ --type=release --python=/usr/local/bin/python --cmake-options='-DBUILDNAME=RDR-MacOS-10.10-clang3.5-release -DSITE=merzbow' 
